...
const slice = createSlice({
  ...
  reducers: {
    ...
    addWindowsWithCurrencyAC(
      state,
      action: PayloadAction<{ element: WindowsWithCurrencyType }>
    ) {
      state.windowsWithCurrency.push(action.payload.element);
    },
  },
});
    ...
    for (let i = 0; i < currency.length; i++) {
      if (newArray.has(currency[i].currency)) {
        const object = {
          ...
          selectedCurrency: { i, currency: currency[i].currency },
        } as any;
        dispatch();
        break;
      }
