...
export const setSelectedPageWindowsWithCurrencyTC =
  (
    windowsWithCurrency: Array<WindowsWithCurrencyType>,
    indexWindow: number,
    index: number,
  ...
  (dispatch: Dispatch) => {
    ...
    if (!newArray.has(currency)) {
      dispatch(
        setSelectedPageWindowsWithCurrencyAC({ indexWindow, index, currency })
      );
    }
    const checkSelectedCurrencies = (
      windowsWithCurrency: Array<WindowsWithCurrencyType>,
      currency: string
    ) => {
      const newArray = new Map();
