  ...
  (dispatch: Dispatch) => {
    ...
    for (let i = 0; i < currency.length; i++) {
      if (!newArray.has(currency[i].currency)) {
        const ratio =
          windowsWithCurrency[0].ratio / windowsWithCurrency[i].ratio;
        const object = {
          value:
            String((Number(windowsWithCurrency[0]) * ratio).toFixed(2)) || "",
...
export const setValuesWindowsWithCurrencyTC =
  ...
  (dispatch: Dispatch) => {
    const newArray: Array<WindowsWithCurrencyType> = [];
    windowsWithCurrency.forEach((el) => {
      const object = { ...el, selectedCurrency: { ...el.selectedCurrency } };
      newArray.push(object);
    });
