...
const slice = createSlice({
  ...
  reducers: {
    ...
    setSelectedPageWindowsWithCurrencyAC(
      ...
      const ratio =
        state.windowsWithCurrency[0].ratio /
        (state.currency[index].rateBuy || state.currency[index].rateCross || 1);
      state.windowsWithCurrency[indexWindow].ratio =
        state.currency[index].rateBuy || state.currency[index].rateCross || 1;
      state.windowsWithCurrency[indexWindow].value = String(
        (Number(state.windowsWithCurrency[0].value) * ratio).toFixed(2)
      );
...
export const setSelectedCurrencyTC =
  ...
  (dispatch: Dispatch) => {
    ...
    const object = { ...windowsWithCurrency[indexWindow], value };
  };
