...
const slice = createSlice({
  ...
  reducers: {
    ...
    setSelectedCurrencyAC(
      ...
      state.windowsWithCurrency.splice(index, 1, object);
    },
    setSelectOpenAC(
      state,
      action: PayloadAction<{
      ...
      state.selectOpen = action.payload.array;
    },
    addSelectOpenAC(state) {
      state.selectOpen.push(false);
    },
    deleteSelectOpenAC(
      state,
      action: PayloadAction<{
      ...
      state.selectOpen.splice(action.payload.index, 1);
    },
    addErrorAC(
      state,
      action: PayloadAction<{
...
export const createWindowsWithCurrencyTC =
  (currency: Array<CurrencyType>) => (dispatch: Dispatch) => {
    ...
    dispatch(setSelectOpenAC({ array }));
  };
  ...
  (dispatch: Dispatch) => {
    if (
      !valueValidator(
        value,
        dispatch,
        addErrorAC({ error: "Максимальное значение 1000000" })
      )
    ) {
      return;
    }
    if (value) {
      ...
      newArray.forEach((el: any, index: number) => {
        if (index !== indexWindow) {
          ...
          newArray[index].value = String((Number(value) * windowsWithCurrency[indexWindow].ratio /
          windowsWithCurrency[index].ratio).toFixed(2));
        }
      });
      dispatch(setWindowsWithCurrencyAC({ newArray }));
    } else {
...
export const deleteWindowsWithCurrencyTC =
  (index: number) => (dispatch: Dispatch) => {
    dispatch(deleteWindowsWithCurrencyAC({ index }));
    dispatch(deleteSelectOpenAC({ index }));
  };
export const setSelectedCurrencyTC =
  (
    windowsWithCurrency: Array<WindowsWithCurrencyType>,
    stateCurrency: Array<CurrencyType>,
    indexWindow: number,
    index: number,
    currency: string
  ) =>
    ...
    const value = windowsWithCurrency[0].value
      ? String(
          (
            ...
            (windowsWithCurrency[0].ratio /
              (stateCurrency[index].rateBuy ||
                stateCurrency[index].rateCross ||
                1))
          ).toFixed(2)
        )
      : "";
    const ratio =
      stateCurrency[index].rateBuy || stateCurrency[index].rateCross || 1;
    const symbol = stateCurrency[index].symbol;
    const selectedCurrency = { index, currency, symbol };
    const object = {
      ...windowsWithCurrency[indexWindow],
      value,
      ratio,
      selectedCurrency,
    };
...
export const setSelectOpenTC =
  (selectOpen: Array<boolean>, index: number | null, value: boolean) =>
  (dispatch: Dispatch) => {
    const array: Array<boolean> = [];
    selectOpen.forEach((el, indexEl) => {
      if (indexEl === index) {
        array.push(value);
      } else {
        array.push(false);
