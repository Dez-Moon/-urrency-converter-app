...
const ConversionBlock = React.memo((props: PropsType) => {
  ...
  useEffect(() => {
    ...
    if (props.currency[selectedValue].rateBuy) {
      newArray[props.index].ratio = props.currency[selectedValue].rateBuy;
    } else if (props.currency[selectedValue].rateCross) {
      newArray[props.index].ratio = props.currency[selectedValue].rateCross;
    } else {
        ...
        onChange={(e: ) => {
          ...
          arrayCopy.forEach((el, index) => {
            if (index !== props.index) {
                if (props.values[props.index].ratio)
                arrayCopy[index].value =
                  String(props.values[props.index].ratio * e.target.value)
            }
          });
