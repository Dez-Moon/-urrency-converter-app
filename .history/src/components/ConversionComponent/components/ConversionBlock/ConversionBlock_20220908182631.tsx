...
const ConversionBlock = React.memo((props: PropsType) => {
  ...
  const inputMargin = `${String(
    50 - windowsWithCurrency[props.index].value.length * (windowsWithCurrency[props.index].selectedCurrency.currency === 'CZK' ? 5: 4
  )}px`;
  const inputStyle = {
    right: inputMargin,
  };
  ...
  return (
    <div className='conversionBlock'>
      <div
        ...
        onClick={async () => {
          setTimeout(() => {
            if (selectOpen[props.index])
              setSelectOpenTC(selectOpen, props.index, false)(dispatch);
            else setSelectOpenTC(selectOpen, props.index, true)(dispatch);
          }, 0);
        }}
      >
