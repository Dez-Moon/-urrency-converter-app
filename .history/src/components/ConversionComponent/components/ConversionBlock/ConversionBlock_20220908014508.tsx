...
const ConversionBlock = (props: PropsType) => {
  const { currency, windowsWithCurrency } = useSelector<AppRootStateType>(
    ...
    windowsWithCurrency: WindowsWithCurrencyType;
  };
    ...
    <div className='conversionBlock'>
      <div className='selectedCurrency' onClick={handleClickToCurrency}>
        <div className='flagAndAbbreviation'>
          <img
            src={
              currency[windowsWithCurrency[props.index].selectedCurrency].flag
            }
          />
          <div>{currency[windowsWithCurrency.selectedCurrency].currency}</div>
        </div>
        <div>{currency[windowsWithCurrency.selectedCurrency].name}</div>
        {!selectOpen ? (
