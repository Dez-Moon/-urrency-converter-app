...
const ConversionBlock = React.memo((props: PropsType) => {
  ...
  useEffect(() => {
    ...
    const getRatio = (index: number) => {
      if (index === 0) {
        return {value: props.values[index+1].value, ratio: props.values[index].ratio / props.values[index + 1].ratio}
      } else {
        return {value: props.values[index+-].value,ratio:props.values[index - 1].ratio / props.values[index].ratio}
      }
    };
    const ratio = getRatio(props.index);
    debugger;

      newArray[props.index].value = String(
        (Number(el.value) / ratio).toFixed(2)
      );
