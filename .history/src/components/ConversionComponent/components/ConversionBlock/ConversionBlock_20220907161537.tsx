...
const getDefaultValue = (values: ValuesType, currency: Array<CurrencyType>) => {
  debugger;
  let selectedValue = values[1];
  let concurrence = 0;
  const a = new Map();
  values.forEach((el) => {
    if (el.currency === currency[index].currency) {
      concurrence++;
    } else {
...
const ConversionBlock = React.memo((props: PropsType) => {
  ...
  const [selectedValue, setSelectedValue] = useState(
    getDefaultValue(props.values, props.currency)
  ) as any;
