...
export const getWindowsChangedListOfCurrenciesOpen = (
  windowsWithCurrency: Array<WindowsWithCurrencyType>,
  indexWindow: number | null
) => {
  const newArray: Array<WindowsWithCurrencyType> = [];
  windowsWithCurrency.forEach((currency, index) => {
    const object: WindowsWithCurrencyType = {
      ...
      selectedCurrency: {
        ...
        index: currency.selectedCurrency.index,
        currency: currency.selectedCurrency.currency,
        symbol: currency.selectedCurrency.symbol,
      },
      listOfCurrenciesOpen:
        indexWindow === index ? !currency.listOfCurrenciesOpen : false,
    };
    newArray.push(object);
  });
  return newArray;
