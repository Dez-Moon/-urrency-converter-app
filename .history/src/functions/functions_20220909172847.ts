...
export const getIndentForValues = (
  currency: Array<{ currency: string; value: number }>,
  currentCurrency: string,
  indexCurrentCurrency: number,
  windowsWithCurrency: Array<WindowsWithCurrencyType>
) => {
  currency.forEach((el) => {
    const indent = el.currency === currentCurrency ? el.value : 4;
    const value =
      55 - windowsWithCurrency[indexCurrentCurrency].value.length * indent;
    debugger;
    return `${value}px`;
  });
};
const getTypeForCurrencyFromData = (
  currencyData: Map<string, CurrencyType>
) => {
  const newArray: any = [];
  currencyData.forEach((value, key) => {
    newArray.push(value.currency);
  });
  return newArray;
};
const a = getTypeForCurrencyFromData(currencyData);
export type Type2 = typeof a;
