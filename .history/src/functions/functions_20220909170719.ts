...
export const getIndentForValues = (
  currency: Array<{ currency: string; values: string }>,
  currentCurrency: string,
  ...
  `${String(
    55 -
      windowsWithCurrency[indexCurrentCurrency].value.length *)
  )}px`;
  currency.forEach((el) => {
    let value = 55 - windowsWithCurrency[indexCurrentCurrency].value.length * (el.currency === currentCurrency ? el.value : 4) 
    return String(value)
  });
};
